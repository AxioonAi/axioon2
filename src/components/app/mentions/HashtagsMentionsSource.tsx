...
interface HashtagsMentionsSourceDataProps {
  name: string;
  quantity: number;
  sentiment: number;
}

export function HashtagsMentionsSource() {
  const [mentionsBySource, setMentionsBySource] = useState<
    HashtagsMentionsSourceDataProps[]
  >([]);
  ...
  useEffect(() => {
    if (hashtagData) {
      setMentionsBySource(hashtagData.hashtagMentions.mentionsByFount);
    }
  }, [hashtagData]);
