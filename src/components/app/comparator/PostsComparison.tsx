...
export function PostsComparison() {
  const {
    activeUserData,
    passiveUserData,
    ...
    if (activeUserData) {
      setActiveFacebookFollowers(
        (activeUserData.posts.facebook &&
          activeUserData.posts.facebook.length) ||
          null,
      );
      setActiveInstagramFollowers(
        (activeUserData.posts.instagram &&
          activeUserData.posts.instagram.length) ||
          null,
      );
      setActiveTiktokFollowers(
        (activeUserData.posts.tiktok && activeUserData.posts.tiktok.length) ||
          null,
      );
      setActiveYoutubeFollowers(
        (activeUserData.posts.youtube && activeUserData.posts.youtube.length) ||
          null,
      );
    }
  }, [activeUserData]);
    ...
    if (passiveUserData) {
      setPassiveFacebookFollowers(
        (passiveUserData.posts.facebook &&
          passiveUserData.posts.facebook.length) ||
          null,
      );
      setPassiveInstagramFollowers(
        (passiveUserData.posts.instagram &&
          passiveUserData.posts.instagram.length) ||
          null,
      );
      setPassiveTiktokFollowers(
        (passiveUserData.posts.tiktok && passiveUserData.posts.tiktok.length) ||
          null,
      );
      setPassiveYoutubeFollowers(
        (passiveUserData.posts.youtube &&
          passiveUserData.posts.youtube.length) ||
          null,
      );
    }
  }, [passiveUserData]);
      ...
      <div className="relative flex w-full flex-col items-center justify-between border-b border-b-zinc-700/50 px-4 py-2 xl:px-8 xl:py-4">
        ...
        <div className="flex w-full items-center justify-between">
          ...
          <strong className="hidden text-sm lg:text-base xl:block 2xl:text-lg">
            TOTAL DE POSTS ATUAIS NAS PLATAFORMAS
          </strong>
          <strong className="text-xs lg:text-sm 2xl:text-base">
